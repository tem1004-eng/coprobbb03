...
// 파일명을 위한 상세 타임스탬프 (YYYY-MM-DD_HH시MM분)
const nowTimestampString = () => {
    const now = new Date();
    const datePart = now.toISOString().slice(0, 10);
    const hours = String(now.getHours()).padStart(2, '0');
    const minutes = String(now.getMinutes()).padStart(2, '0');
    return `${datePart}_${hours}시${minutes}분`;
};
  ...
  const handleSaveData = () => {
    const performSave = () => {
      try {
        ...
        // 요청대로 파일명 형식을 [교회이름 헌금관리]_날짜_시간 으로 변경
        const fileName = `[${churchName} 헌금관리]_${nowTimestampString()}.json`;
...
const ExcelExportModal: React.FC<{
    ...
    const handleExport = () => {
        ...
        // 엑셀 파일명 형식도 [교회이름 헌금관리]_구분_날짜_시간.xlsx 로 변경
        const fileName = `[${churchName} 헌금관리]_${mode}_${nowTimestampString()}.xlsx`;
